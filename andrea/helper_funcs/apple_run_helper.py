    ...
    *,
    ...
    if init in {"sample_size", "p0"}:
        for _ in range(num_clients):
            out.append(torch.nn.Parameter(p0.detach().clone()))
        return out

    if init == "uniform":
        base = torch.full_like(p0, 1.0 / max(num_clients, 1))
        for _ in range(num_clients):
            out.append(torch.nn.Parameter(base.detach().clone()))
        return out
...
def mixed_parameter_dict(
    ...
    """
    Differentiably build theta_i^p = sum_j p_ij theta_j^c.

    Only parameters of client_idx remain attached to autograd. All other core
    models are treated as downloaded/frozen constants for this local step.
    """
    ...
    for name in names:
        ...
        for donor_idx, params in enumerate(param_dicts):
            ...
            if donor_idx != client_idx:
                value = value.detach()
            coef = dr_vector[donor_idx].to(device=value.device, dtype=value.dtype)
            term = coef * value
...
def train_epoch_neighbor_apple(
    ...
    *,
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        prox_loss = (
            0.5 * float(lambda_r) * float(apple_mu) * torch.sum((dr_vector - p0) ** 2)
        )
    ...
    rows.append(
        {
            ...
            "dr_row_sum": float(dr_cpu.sum().item()),
            "dr_l2_to_p0": float(torch.norm(dr_cpu - p0_cpu, p=2).item()),
            "dr_self_weight": (
                float(dr_cpu[int(client_idx)].item())
                if dr_cpu.numel() > int(client_idx)
                else None
            ),
...
def append_apple_dr_rows(
    ...
    *,
    ...
    for i, runtime_i in enumerate(runtime_clients):
        row = matrix[i]
        off_mask = torch.ones(n, dtype=torch.bool)
        off_mask[i] = False
        off = row[off_mask]
        ...
        top_parts = []
        for donor_idx in top_order[: min(3, n)]:
            ...
            dr_rows.append(
                {
                    ...
                    "p_ij": float(row[j].item()),
                    "abs_p_ij": float(abs(row[j].item())),
...
def run_apple_experiment(
    ...
    *,
    ...
    # For fair comparison with the current FedAvg/FedProx/GCFL+ code, all core
    # models start from the same random initialization, then become personalized.
