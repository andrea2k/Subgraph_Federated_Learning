...
from andrea.helper_funcs_multihead.apple_backbone_task_head_run_helper import (
    build_apple_log_row,
    run_apple,
)
...
EXPERIMENT_LOG_FOLDER = "apple_backbone_taskhead_clustering_experiment"
...
#
# Current backbone+task-head APPLE setting.
# You can change these independently from old APPLE / head-only APPLE runs.
# -----------------------------------------------------------------------------
APPLE_DR_LR = 1e-3
APPLE_MU = 1e-3
APPLE_SCHEDULER_TYPE = "cosine"
...
def main():
    ...
    print("\nBACKBONE+TASK-HEAD APPLE CONFIG")
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            print()
            print(
                f"Starting with {len(subset_clients)}-client APPLE backbone+task-head training",
                row["subset_clients"],
            )
            print(meta)
            print(
