...
from andrea.helper_funcs_multihead.spec_select_train_bcast_run_helper_multi_select import (
    run_spec_select_train_bcast,
)
...
RUN_TAG = os.environ.get("RUN_TAG", "spec_select_train_bcast_v1")
BASE_EXPERIMENT_LOG_FOLDER = "spec_select_train_bcast_experiment_multi_select"
...
RUNS_ROOT = os.environ.get(
    "RUNS_ROOT", "andrea/runs_spec_select_train_bcast"
)
...
RESULT_RUN_TYPE = "spec_select_train_bcast_apple_post_ala_support_simplex_multi_select"
RESULT_ALGORITHM = "spec_select_train_bcast_apple_post_ala_support_simplex_multi_select"
DISPLAY_NAME = "Specialist Selection + Dominant Training + Broadcast"
...
def main() -> None:
    ...
    if BENCHMARK.setup != "twenty_client":
        raise ValueError(
            "SpecSelectTrainBcast is 20-client-only. "
    ...
    print("REPRESENTATIVE SELECTION: argmax visible TRAIN support within known specialist group")
    print("ACTIVE TRAINING CLIENTS: 5 / 20")
    print("DEPLOYMENT: full-model broadcast to all 20 clients")
    ...
    base_cfg["apple_mixing_mode"] = "five_rep_fedavg_postala_full_model_broadcast"
    base_cfg["selection_tag"] = (
        "round0_argmax_visible_train_support_known_specialist_groups"
    )
    ...
    base_cfg["apple_routing_mode"] = "five_rep_support_simplex"
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            paths = run_spec_select_train_bcast(
                subset_clients,
                cfg,
                seed,
                RUNS_ROOT,
            ...
            log_row.update(
                {
                    ...
                    "specialist_warmup_rounds": 0,
                    "representative_selection_rule":
                        "argmax_visible_train_support_within_known_specialist_group",
                    "known_specialist_grouping": True,
                    "active_training_clients": 5,
                    "total_clients": 20,
                    "specialist_num_donors": 5,
                    "specialist_representatives_csv":
                        str(paths.representatives_csv_path),
                    "backbone_aggregation_clients": 5,
                    "task_head_donor_clients": 5,
                    "dr_expected_shape": "5x5x5",
                    "full_model_broadcast": True,
                    "deployment_clients": 20,
                    "recipient_finetuning": False,
                }
            )
            upsert_experiment_rows(experiment_log_csv, [log_row])
            progress.step(
                label=f"SpecSelectTrainBcast done -> {paths.csv_path}",
