...
from andrea.helper_funcs_multihead.apple_task_head_run_helper import (
    build_apple_log_row,
    run_apple,
)
...
EXPERIMENT_LOG_FOLDER = "apple_taskhead_clustering_experiment"
...
#
# New APPLE diagnostic setting:
#   stronger DR proximal penalty + longer scheduler window.
# -----------------------------------------------------------------------------
APPLE_DR_LR = 1e-3
APPLE_MU = 1e-3
APPLE_SCHEDULER_TYPE = "cosine"
...
def main():
    ...
    # Task-head APPLE-specific architecture/mixing flags.
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            print()
            print(
                f"Starting with {len(subset_clients)}-client APPLE training",
                row["subset_clients"],
            )
            print(meta)
            print(
