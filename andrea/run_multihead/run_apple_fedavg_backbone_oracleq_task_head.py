...
from andrea.helper_funcs_multihead.apple_fedavg_backbone_oracleq_task_head_run_helper import (
    build_apple_log_row,
    run_apple,
)
...
EXPERIMENT_LOG_FOLDER = "apple_fedavg_backbone_oracleq_taskhead_clustering_experiment"
...
# -----------------------------------------------------------------------------
# Oracle q-weighted task-head setup.
#
# Initial APPLE setting, kept here for reference:
#   APPLE_DR_LR = 1e-3
#   APPLE_MU = 1e-3
...
#
# Current FedAvg-backbone + fixed oracle-q task-head setting.
# -----------------------------------------------------------------------------
APPLE_DR_LR = 0.0
APPLE_MU = 0.0
APPLE_SCHEDULER_TYPE = "cosine"
APPLE_SCHEDULER_FRACTION = 0.2
APPLE_DR_INIT = "oracle_q"
APPLE_DR_CONSTRAINT = "fixed_oracle_q"
APPLE_DOWNLOAD_STRATEGY = "full"
...
def main():
    ...
    # Oracle-q task-head architecture/mixing flags.
    ...
    print("\nFEDAVG-BACKBONE + ORACLE-Q TASK-HEAD CONFIG")
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            print()
            print(
                f"Starting with {len(subset_clients)}-client fedavg-backbone + oracle-q task-head training",
                row["subset_clients"],
            )
            print(meta)
            print(
            ...
            apple_row["mask_splits"] = "|".join(MASK_SPLITS)
            apple_row["selection_modes"] = "full|visible"
            apple_row["eval_protocols"] = (
