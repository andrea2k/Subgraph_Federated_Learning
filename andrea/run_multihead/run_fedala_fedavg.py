...
EXPERIMENT_LOG_FOLDER = "fedala_fedavg_multihead_clustering_experiment"
...
# FedALA setup.
#
# FEDALA_MODE="all" is the paper-like FedALA-FedAvg variant:
#   - server aggregation is still weighted FedAvg
#   - every learnable parameter is ALA-interpolated client-side
#   - non-learnable buffers are copied from the global model
#
...
def main() -> None:
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            print()
            print(
                f"Starting with {len(subset_clients)}-client FedALA-FedAvg training",
                row["subset_clients"],
            )
            print(meta)
            ...
            progress.step(
                label=f"fedala_fedavg done -> {run_paths.csv_path}",
