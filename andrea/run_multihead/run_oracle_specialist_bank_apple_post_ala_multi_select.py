...
from andrea.helper_funcs_multihead.oracle_specialist_bank_run_helper_multi_select import (
    run_oracle_specialist_bank,
)
...
RUN_TAG = os.environ.get("RUN_TAG", "oracle_specialist_bank_support_simplex_v1")
BASE_EXPERIMENT_LOG_FOLDER = "oracle_specialist_bank_apple_post_ala_experiment_multi_select"
...
RUNS_ROOT = os.environ.get(
    "RUNS_ROOT", "andrea/runs_oracle_specialist_bank_apple_post_ala_support_simplex"
)
...
SPECIALIST_SELECTION_VIEW = os.environ.get("SPECIALIST_SELECTION_VIEW", "visible").strip().lower()
...
RESULT_RUN_TYPE = "oracle_specialist_bank_apple_post_ala_support_simplex_multi_select"
RESULT_ALGORITHM = "oracle_specialist_bank_apple_post_ala_support_simplex_multi_select"
DISPLAY_NAME = "OracleSpecialistBank-APPLE-PostALA Support-Simplex"
...
def main() -> None:
    ...
    if BENCHMARK.setup != "twenty_client":
        raise ValueError(
            "OracleSpecialistBank is 20-client-only. "
            f"Got BENCHMARK_SETUP={BENCHMARK.setup!r}."
        )
    if SPECIALIST_SELECTION_VIEW not in {"visible", "oracle"}:
    ...
    base_cfg["apple_mixing_mode"] = "all20_fedavg_backbone_oracle_specialist_5donor_heads"
    base_cfg["selection_tag"] = (
        f"oracle_specialist_bank_{SPECIALIST_SELECTION_VIEW}_w{WARMUP_ROUNDS}"
    )
    ...
    base_cfg["apple_routing_mode"] = "oracle_specialist_bank_support_simplex"
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            paths = run_oracle_specialist_bank(
                subset_clients,
                cfg,
                seed,
                RUNS_ROOT,
            ...
            log_row.update(
                {
                    ...
                    "specialist_representatives_csv": str(paths.representatives_csv_path),
                    "backbone_aggregation_clients": 20,
                    "task_head_donor_clients": 5,
                    "dr_expected_shape": "20x5x5",
                }
            )
            upsert_experiment_rows(experiment_log_csv, [log_row])
            progress.step(
                label=f"OracleSpecialistBank done -> {paths.csv_path}",
