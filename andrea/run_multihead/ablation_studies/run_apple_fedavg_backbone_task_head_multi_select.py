...
from andrea.helper_funcs_multihead.ablation_studies.apple_fedavg_backbone_task_head_run_helper_multi_select import (
    build_apple_log_row,
    run_apple,
)
...
BASE_EXPERIMENT_LOG_FOLDER = "apple_fedavg_backbone_taskhead_clustering_experiment_multi_select"
...
RESULT_RUN_TYPE = "apple_fedavg_backbone_taskhead_multi_select"
RESULT_ALGORITHM = "apple_fedavg_backbone_taskhead_multi_select"
DISPLAY_NAME = "APPLE FedAvg-Backbone+TaskHead (multi-selection)"
...
def main() -> None:
    print("RUN VARIANT: APPLE-FedAvgBackboneTaskHead (multi-selection) | selection_metrics=" + "|".join(SELECTION_METRICS))
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            print()
            print(f"Starting with {len(subset_clients)}-client APPLE-FedAvgBackboneTaskHead training", row["subset_clients"])
            print(meta)
            ...
            progress.step(label=f"APPLE-FedAvgBackboneTaskHead done -> {run_paths.csv_path}", run_start_time=run_start)
