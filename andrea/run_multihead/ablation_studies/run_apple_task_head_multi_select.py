...
from andrea.helper_funcs_multihead.ablation_studies.apple_task_head_run_helper_multi_select import (
    build_apple_log_row,
    run_apple,
)
...
BASE_EXPERIMENT_LOG_FOLDER = "apple_taskhead_clustering_experiment_multi_select"
...
RESULT_RUN_TYPE = "apple_taskhead_multi_select"
RESULT_ALGORITHM = "apple_taskhead_multi_select"
DISPLAY_NAME = "APPLE TaskHead (multi-selection)"
...
def main() -> None:
    print(
        "RUN VARIANT: APPLE-TaskHead (multi-selection) | selection_metrics="
    ...
    base_cfg["apple_mixing_mode"] = "task_head"
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            print()
            print(
                f"Starting with {len(subset_clients)}-client APPLE-TaskHead training",
                row["subset_clients"],
            )
            print(meta)
            ...
            progress.step(
                label=f"APPLE-TaskHead done -> {run_paths.csv_path}",
