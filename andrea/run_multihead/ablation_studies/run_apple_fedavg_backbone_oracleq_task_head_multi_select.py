...
from andrea.helper_funcs_multihead.ablation_studies.apple_fedavg_backbone_oracleq_task_head_run_helper_multi_select import (
    build_apple_log_row,
    run_apple,
)
...
BASE_EXPERIMENT_LOG_FOLDER = "apple_fedavg_backbone_oracleq_taskhead_clustering_experiment_multi_select"
...
APPLE_DR_LR = float(os.environ.get("APPLE_DR_LR", 0.0))
APPLE_MU = float(os.environ.get("APPLE_MU", 0.0))
...
APPLE_DR_INIT = os.environ.get("APPLE_DR_INIT", "oracle_q")
APPLE_DR_CONSTRAINT = os.environ.get("APPLE_DR_CONSTRAINT", "fixed_oracle_q")
...
RESULT_RUN_TYPE = "apple_fedavg_backbone_oracleq_taskhead_multi_select"
RESULT_ALGORITHM = "apple_fedavg_backbone_oracleq_taskhead_multi_select"
DISPLAY_NAME = "APPLE FedAvg-Backbone+OracleQ TaskHead (multi-selection)"
...
def main() -> None:
    print("RUN VARIANT: APPLE-FedAvgBackboneOracleQTaskHead (multi-selection) | selection_metrics=" + "|".join(SELECTION_METRICS))
    ...
    base_cfg["apple_mixing_mode"] = "fedavg_backbone_oracleq_task_head"
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            print()
            print(f"Starting with {len(subset_clients)}-client APPLE-FedAvgBackboneOracleQTaskHead training", row["subset_clients"])
            print(meta)
            ...
            progress.step(label=f"APPLE-FedAvgBackboneOracleQTaskHead done -> {run_paths.csv_path}", run_start_time=run_start)
