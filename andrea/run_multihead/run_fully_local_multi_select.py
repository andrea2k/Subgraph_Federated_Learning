...
BASE_EXPERIMENT_LOG_FOLDER = "fully_local_multihead_clustering_experiment_multi_select"
...
RESULT_RUN_TYPE = "fully_local_multi_select"
RESULT_ALGORITHM = "fully_local_multi_select"
DISPLAY_NAME = "Fully local (multi-selection)"
...
def relabel_result_csv(
    ...
    """
    run_local(...) internally writes rows as run_type='local'.
    For the renamed baseline, patch the result CSV too, not only the manifest row.
    """
...
def main():
    ...
    print(
        "RUN VARIANT: Fully local (multi-selection) | selection_metrics=loss|micro_f1|macro_pos_f1|micro_pr_auc|macro_pr_auc"
    )
    ...
    for cfg, meta in sweep:
        ...
        for _, row in chosen_df.iterrows():
            ...
            graph_to_family = {}
            if (
                "graph_to_family_json" in row.index
                and pd.notna(row["graph_to_family_json"])
            ):
            ...
            print()
            print(
                f"Starting fully-local masked-client training",
                row["subset_clients"],
            )
            print(meta)
            ...
            for graph_id in subset_id:
                ...
                family = graph_to_family.get(str(graph_id))
                if family is None:
                    assigned_task = (
                        mask_meta.get("assigned_task")
                        or mask_meta.get("mask_task")
                    )
                    family = (
                        f"q_task_{assigned_task}"
                        if assigned_task
                        else str(row.get("family", "unknown"))
                    )
                print(
                ...
                if os.environ.get("DEBUG_CLIENT", "0") == "1":
                    print_client_debug_summary(
                        client,
                        name=f"FULLY_LOCAL | graph={graph_id} | family={family}",
                    )
                ...
                local_log_row["training_scope"] = "masked_virtual_client_graph"
                ...
                progress.step(
                    label=f"fully_local done -> {local_paths.csv_path}",
