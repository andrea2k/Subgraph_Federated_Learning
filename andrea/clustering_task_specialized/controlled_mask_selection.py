...
OUT_DIR = "./andrea/clustering_task_specialized"
SPLIT = "train"
...
FAMILY_PREFIX = "specialized"

BASE_TARGET_POS_RATE = 0.05
BASE_MIN_POS_RATE = 0.01

# Avoid graph_id collisions with old registry.
VIRTUAL_GRAPH_ID_START = 2_000_000

# Mask metadata.
MASK_MODE = "keep_assigned_task_mask_other_labels"
MASK_APPLY_SPLIT = "train"
MASK_SEED_BASE = 0

CONTROLLED_BENCHMARK = "task_specialized_label_masking"
    ...
    *,
    ...
    for task in TASKS:
        ...
        if task == assigned_task:
            ...
            visible_rate = true_rate * (1.0 - float(mask_fraction))
            visible_pos_nodes = true_pos_nodes * (1.0 - float(mask_fraction))
...
def make_virtual_client_row(
    ...
    *,
    ...
    virtual_dataset_id = f"{base_dataset_id}_specialized{x_tag}_{assigned_task}"
    ...
    # Compute expected visible support after task-specialized masking.
    ...
    # Make train_* columns represent the effective visible training profile.
    ...
    masked_tasks = [task for task in TASKS if task != assigned_task]

    # Metadata consumed later by the loader.
    row["mask_mode"] = MASK_MODE
    row["assigned_task"] = assigned_task

    # Keep mask_task for backward-compatible naming.
    # In this new design, mask_task means the assigned/kept task.
    row["mask_task"] = assigned_task

    row["masked_tasks_json"] = safe_json_dumps(masked_tasks)
...
def build_manifest_row_for_group(
    ...
    *,
    ...
    subset_id = (
        f"controlled_specialized_base{base_rank}_x{mask_fraction}_"
    ...
    global_visible_support_fraction_ideal = (
        1.0 + (num_tasks - 1) * (1.0 - float(mask_fraction))
    ) / num_tasks

    row = {
        "family": "controlled_task_specialized_five_client_benchmark",
        "subset_id": subset_id,
        ...
        "specialization_fraction": float(mask_fraction),
...
def print_manifest_summary(manifest_df: pd.DataFrame) -> None:
    ...
    print("\n" + "=" * 100)
    print("TASK-SPECIALIZED MASKED SELECTED SUBSET SUMMARY")
    print("=" * 100)
