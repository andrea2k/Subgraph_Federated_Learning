...
def compact_subset_id_for_filename(subset_id: str) -> str:
    """Return a short, readable subset id that is safe for macOS/Linux filenames."""
    raw = str(subset_id)
    parts = [p for p in raw.split("|") if p]

    # Readable special case for the q-controlled benchmark client ids.
    try:
...
def compact_model_tag_for_filename(model_tag: str) -> str:
    """Shorten only the very long multi-select suffix; keep model hyperparams readable."""
    tag = str(model_tag)
    replacements = {
...
def create_gcfl_run_paths(
    ...
    # Multi-selection GCFL+ does not create/use andrea/checkpoints by default.
    ckpt_root = ensure_dir(gcfl_root)
    subset_tag = compact_subset_id_for_filename(subset_id)
    model_tag = compact_model_tag_for_filename(model_tag)
    stem = (
        f"gcflplus_{subset_tag}"
        f"_r{rounds}"
        f"_e{local_epochs}"
        f"_w{warmup_rounds}"
        f"_h{grad_seq_len}"
        f"_mcs{min_cluster_size}"
        f"_mch{min_child_size}"
        f"_e1q{eps1_quantile}"
        f"_e2q{eps2_quantile}"
        f"_{model_tag}"
        f"_s{seed}"
    )
    return GcflRunPaths(
...
def run_gcfl_experiment(
    ...
    *,
    ...
    for round_idx in range(1, rounds + 1):
        ...
        for eval_mask_mode in ("full", "visible"):
            ...
            for selector_metric in selection_metrics:
                current_value = mean_scalar.get(selector_metric)
                ...
                if is_better_selection_value(
                    ...
                    best_record["round"] = int(round_idx)
                    best_record["clusters_snapshot"] = snapshot_clusters(clusters)
    ...
    # Optional metadata checkpoint only when explicitly requested.
    best_loss_full = (
        best_by_eval_mode["full"].get("loss", {}).get("value")
        if "loss" in best_by_eval_mode["full"]
        else None
    )
    ...
    checkpoint = {
        ...
        "best_round_by_eval_mode_and_metric": {
            mode: {metric: rec["round"] for metric, rec in by_metric.items()}
            for mode, by_metric in best_by_eval_mode.items()
        },
        "best_value_by_eval_mode_and_metric": {
            mode: {metric: rec["value"] for metric, rec in by_metric.items()}
            for mode, by_metric in best_by_eval_mode.items()
        },
        "eval_protocols": [
            "oracle_full",
    ...
    for selector_metric in selection_metrics:
        for job in final_eval_jobs:
            ...
            selected_clusters = restore_clusters(selected_record["clusters_snapshot"])
