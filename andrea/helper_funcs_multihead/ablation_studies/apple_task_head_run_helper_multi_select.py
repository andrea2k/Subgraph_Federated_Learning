...
def build_apple_log_row(
    *,
    ...
    return {
        "run_type": "apple_taskhead_multi_select",
        "algorithm": "apple_taskhead_multi_select",
        "subset_id": subset_id,
        ...
        "apple_mixing_mode": cfg.get("apple_mixing_mode", "task_head"),
...
def dr_matrix_cpu(
    dr_vectors: Sequence[torch.nn.Parameter | torch.Tensor],
) -> torch.Tensor:
    return torch.stack([dr.detach().cpu().clone() for dr in dr_vectors], dim=0)
...
def initialize_dr_vectors(
    ...
    *,
    ...
    """
    Task-head APPLE DR initialization.

    Old APPLE:
        one DR vector per client
        shape per client = [num_clients]

    Task-head APPLE:
        one DR vector per client per task
        shape per client = [num_tasks, num_clients]

    For each client i:
        dr_vectors[i][task_idx, donor_idx] = p_{i, task, donor}
    """
    init = str(init).lower().strip()
    out: List[torch.nn.Parameter] = []

    if init in {"sample_size", "p0"}:
        base = p0.detach().clone().unsqueeze(0).repeat(int(num_tasks), 1)
        for _ in range(num_clients):
            out.append(torch.nn.Parameter(base.clone()))
        return out

    if init == "uniform":
        uniform = torch.full_like(p0, 1.0 / max(num_clients, 1))
        base = uniform.detach().clone().unsqueeze(0).repeat(int(num_tasks), 1)
        for _ in range(num_clients):
            out.append(torch.nn.Parameter(base.clone()))
        return out
...
def mixed_parameter_dict(
    ...
    client_idx: int,
    dr_task_matrix: torch.Tensor,
) -> Dict[str, torch.Tensor]:
    """
    Build the personalized task-head APPLE parameter dictionary.

    Important difference from vanilla APPLE:

    - Backbone parameters are NOT mixed.
      They come only from the receiver/client's own local model.

    - Task-head parameters ARE mixed task-wise:
          head_{i,t} = sum_j p_{i,t,j} * head_{j,t}

    This means each task can learn a different donor pattern.
    """
    if len(models_for_mix) == 0:
        raise ValueError("models_for_mix is empty")

    if dr_task_matrix.ndim != 2:
        raise ValueError(
            f"Expected dr_task_matrix shape [num_tasks, num_clients], "
            f"got shape={tuple(dr_task_matrix.shape)}"
        )

    num_tasks, num_clients_from_dr = dr_task_matrix.shape
    if num_clients_from_dr != len(models_for_mix):
        raise ValueError(
            f"DR matrix donor dimension mismatch: "
            f"dr has {num_clients_from_dr}, models_for_mix has {len(models_for_mix)}"
        )
    ...
    for name in names:
        ...
        # Backbone / non-head parameters:
        # keep receiver-local and attached to autograd.
        if task_idx is None:
            mixed[name] = param_dicts[client_idx][name]
            continue
        ...
        # Task-head parameter:
        # mix only same-task heads across donors.
        acc = None
        for donor_idx, params in enumerate(param_dicts):
            value = params[name]

            # Donor models are frozen/downloaded constants.
            # Receiver client's own head remains attached to autograd.
            if donor_idx != client_idx:
                value = value.detach()

            coef = dr_task_matrix[task_idx, donor_idx].to(
                device=value.device,
                dtype=value.dtype,
            )
            term = coef * value
...
def train_epoch_neighbor_apple(
    ...
    *,
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        # dr_vector is now actually a task DR matrix:
        # shape [num_tasks, num_clients].
        #
        # Compare every task-specific DR row to p0, then average over tasks.
        # We use mean over tasks so apple_mu remains roughly comparable to
        # vanilla APPLE instead of becoming 5x stronger automatically.
        p0_task = p0.unsqueeze(0).to(device=dr_vector.device, dtype=dr_vector.dtype)
        diff = dr_vector - p0_task
        prox_loss = (
            0.5
            * float(lambda_r)
            * float(apple_mu)
            * torch.mean(torch.sum(diff**2, dim=1))
        )
...
def append_apple_train_diag_row(
    ...
    *,
    ...
    """
    Training diagnostic row for task-head APPLE.

    dr_vector is shape [num_tasks, num_clients].
    We log averaged summaries here. Detailed per-task DR values are written
    separately to the DR csv by append_apple_dr_rows().
    """
    dr_cpu = dr_vector.detach().cpu()
    p0_cpu = p0.detach().cpu()

    if dr_cpu.ndim != 2:
        raise ValueError(f"Expected task DR matrix, got shape={tuple(dr_cpu.shape)}")

    row_sums = dr_cpu.sum(dim=1)
    self_weights = dr_cpu[:, int(client_idx)]
    l2_to_p0 = torch.norm(dr_cpu - p0_cpu.unsqueeze(0), p=2, dim=1)
    ...
    rows.append(
        {
            ...
            "dr_row_sum": float(row_sums.mean().item()),
            "dr_l2_to_p0": float(l2_to_p0.mean().item()),
            "dr_self_weight": float(self_weights.mean().item()),
            "dr_row_sum_by_task_json": json.dumps(
                [float(x) for x in row_sums.tolist()]
            ),
            "dr_self_weight_by_task_json": json.dumps(
                [float(x) for x in self_weights.tolist()]
            ),
            "dr_l2_to_p0_by_task_json": json.dumps(
                [float(x) for x in l2_to_p0.tolist()]
            ),
...
def append_apple_dr_rows(
    ...
    *,
    runtime_clients,
    dr_vectors: Sequence[torch.Tensor],
    p0: torch.Tensor,
    subset_id: str,
    subset_clients: str,
    seed: int,
    ...
    """
    DR logging for task-head APPLE.

    matrix shape:
        [receiver_client, task, donor_client]

    Rows written:
        row_type="task_summary":
            one row per receiver-client/task

        row_type="task_pair":
            one row per receiver-client/task/donor-client
    """
    tensor = dr_matrix_cpu(dr_vectors)

    if tensor.ndim != 3:
        raise ValueError(
            f"Expected DR tensor shape [num_clients, num_tasks, num_clients], "
            f"got shape={tuple(tensor.shape)}"
        )

    p0_cpu = p0.detach().cpu()
    num_receivers, num_tasks, num_donors = tensor.shape

    if num_receivers != len(runtime_clients) or num_donors != len(runtime_clients):
        raise ValueError(
            f"DR tensor/client mismatch: tensor={tuple(tensor.shape)}, "
            f"runtime_clients={len(runtime_clients)}"
        )

    incoming_abs = tensor.abs().sum(dim=0)  # [task, donor]
    incoming_signed = tensor.sum(dim=0)  # [task, donor]
            ...
            dr_rows.append(
                {
                    ...
                    "incoming_abs_mass_this_client_task": float(
                        incoming_abs[task_idx, i].item()
                    ),
                    "incoming_signed_mass_this_client_task": float(
                        incoming_signed[task_idx, i].item()
                    ),
                ...
                dr_rows.append(
                    {
                        ...
                        "p_itj": value,
                        # Keep p_ij alias to make quick plotting easier.
                        "p_ij": value,
    ...
    *,
    ...
    for selector_metric in selection_metrics:
        for job in final_eval_jobs:
            ...
            selected_dr_vectors = [
                torch.nn.Parameter(selected_dr_matrix[i].to(device).clone(), requires_grad=False)
