...
def _copy_ala_weights_by_client(
    weights_by_client: Optional[Dict[int, Dict[str, torch.Tensor]]],
) -> Dict[int, Dict[str, torch.Tensor]]:
    if not weights_by_client:
        return {}
    return {
        int(idx): _copy_ala_weights(weights)
        for idx, weights in weights_by_client.items()
    }


# -----------------------------------------------------------------------------
# New guarded / task-level HeadOnly-ALA utilities for APPLE-ALA variants
# -----------------------------------------------------------------------------
def _task_alpha_key(task_idx: int) -> str:
    return f"__task_alpha_{int(task_idx)}"
...
def _clip_alpha_bounds(alpha_min: float, alpha_max: float) -> tuple[float, float]:
    lo = float(alpha_min)
    hi = float(alpha_max)
    if lo < 0.0 or hi > 1.0 or lo > hi:
        raise ValueError(
            f"ALA alpha bounds must satisfy 0 <= min <= max <= 1, got {lo}, {hi}"
        )
    return lo, hi


def _support_aware_task_alpha_init(
    runtime,
    num_tasks: int,
    *,
    alpha_min: float,
    ...
    """Initialize one ALA alpha per task from visible train-positive support.

    Larger visible support means the local task head is more trustworthy, so the
    initial alpha is closer to alpha_min. Smaller visible support means the
    client should initially accept more of the APPLE/TaskExpert transferred head,
    so alpha is closer to alpha_max.

        alpha[t] = alpha_min + (alpha_max-alpha_min) * (1 - support_norm[t])

    This stays inside the conservative [alpha_min, alpha_max] interval.
    """
    ...
    visible_pos = (
        (y * mask).sum(dim=0)[: int(num_tasks)].to(device=device, dtype=torch.float32)
    )
    max_pos = visible_pos.max().clamp_min(1.0)
    support_norm = visible_pos / max_pos
    alpha = float(alpha_min) + (float(alpha_max) - float(alpha_min)) * (
        1.0 - support_norm
    )
    return alpha.clamp(float(alpha_min), float(alpha_max))


def _task_level_weights_from_alpha(
    template_model: torch.nn.Module,
    task_alpha: torch.Tensor,
) -> Dict[str, torch.Tensor]:
    """Expand [num_tasks] alpha into a dict keyed by task-head parameter name."""
    out: Dict[str, torch.Tensor] = {}
    alpha_cpu = task_alpha.detach().cpu().float()
    for name, _param in template_model.named_parameters():
        task_idx = _task_idx_from_output_head_param_name(name)
        if task_idx is None:
            continue
        if task_idx < 0 or task_idx >= int(alpha_cpu.numel()):
            continue
        # Store a scalar alpha. mixed_parameter_dict broadcasts it to the head tensor.
        out[name] = alpha_cpu[int(task_idx)].clone()
    return out
...
def learn_task_level_ala_weights_for_client(
    *,
    client_idx: int,
    runtime,
    cfg: Dict,
    ...
    alpha_min: float,
    alpha_max: float,
    support_aware_init: bool,
    device: torch.device,
    round_idx: int,
    debug: bool,
) -> tuple[Dict[str, torch.Tensor], Dict[str, float]]:
    """Learn one conservative HeadOnly-ALA alpha per task for APPLE heads.

    This replaces the very flexible per-parameter FedALA gate by a task-level
    gate. The learned alpha is constrained to [alpha_min, alpha_max]. The final
    APPLE head parameter remains:

        local_head + alpha_task * (apple_mixed_head - local_head)

    alpha_min=0.5 prevents complete rejection of APPLE/TaskExpert transfer.
    """
    alpha_min, alpha_max = _clip_alpha_bounds(alpha_min, alpha_max)

    template_model = clone_model_from_state(incoming_state, cfg, ctx, device)
    ...
    eligible_names = [
        ...
        and _task_idx_from_output_head_param_name(name) is not None
    ]
    if not eligible_names:
        raise ValueError("Task-level APPLE-ALA selected zero task-head parameters.")

    # Initialize one task alpha vector. Reuse prior state after first ALA update.
    stored_alpha = []
    have_all_stored = True
    for task_idx in range(int(num_tasks)):
        key = _task_alpha_key(task_idx)
        if key in client_state.weights:
            stored_alpha.append(client_state.weights[key].detach().float().reshape(()))
        else:
            have_all_stored = False
            break

    if have_all_stored:
        init_alpha = torch.stack(stored_alpha).to(device=device, dtype=torch.float32)
        init_source = "previous"
    elif support_aware_init:
        init_alpha = _support_aware_task_alpha_init(
            runtime,
            int(num_tasks),
            alpha_min=alpha_min,
            alpha_max=alpha_max,
        ...
        init_source = "support_aware"
    else:
        init_alpha = torch.full(
            (int(num_tasks),),
            float(alpha_max),
            dtype=torch.float32,
            device=device,
        )
        init_source = "alpha_max"

    init_alpha = init_alpha.clamp(alpha_min, alpha_max)
    task_alpha = torch.nn.Parameter(init_alpha.detach().clone())
    optimizer = torch.optim.SGD([task_alpha], lr=float(ala_lr))
    ...
    # Like FedALA: search more during the start phase, then use one sampled pass.
    ...
    old_incoming_l2 = _head_state_l2(old_local_state, incoming_state, eligible_names)
    ...
    stats: Dict[str, float] = {
        ...
        "ala_mode": "task_level",
        "ala_alpha_min": float(alpha_min),
        "ala_alpha_max": float(alpha_max),
        "ala_init_source": init_source,
        "ala_start_phase": float(was_start_phase),
        "old_global_l2": float(old_incoming_l2),
        "ala_steps": 0.0,
    ...
    def _functional_params() -> Dict[str, torch.Tensor]:
        params: Dict[str, torch.Tensor] = {}
        for name, param in template_model.named_parameters():
            task_idx = _task_idx_from_output_head_param_name(name)
            if task_idx is None:
                params[name] = incoming_state[name].to(device=device, dtype=param.dtype)
                continue
            old_value = old_local_state[name].to(device=device, dtype=param.dtype)
            inc_value = incoming_state[name].to(device=device, dtype=param.dtype)
            alpha = (
                task_alpha[int(task_idx)]
                .clamp(alpha_min, alpha_max)
                .to(dtype=param.dtype)
            )
            params[name] = old_value + alpha * (inc_value - old_value)
        return params

    buffers = {
        name: incoming_state[name].to(device=device, dtype=buf.dtype)
        for name, buf in template_model.named_buffers()
        if name in incoming_state
    }

    if old_incoming_l2 <= 1e-12:
        ...
        while step_count < hard_step_cap and not converged:
            for batch in runtime.train_loader:
                ...
                optimizer.step()
                with torch.no_grad():
                    task_alpha.clamp_(alpha_min, alpha_max)
                ...
                if was_start_phase and len(losses) >= int(ala_convergence_window):
                    recent = np.asarray(
                        losses[-int(ala_convergence_window) :], dtype=np.float64
                    )
                    if float(np.std(recent)) < float(ala_convergence_std):
                        converged = True
                        break

    with torch.no_grad():
        task_alpha.clamp_(alpha_min, alpha_max)
        final_alpha = task_alpha.detach().cpu().float().clone()

    for task_idx in range(int(num_tasks)):
        client_state.weights[_task_alpha_key(task_idx)] = final_alpha[
            int(task_idx)
        ].clone()
    client_state.start_phase = False

    if losses:
        stats["ala_steps"] = float(step_count)
        stats["ala_first_loss"] = float(losses[0])
        stats["ala_last_loss"] = float(losses[-1])
        if len(losses) >= int(ala_convergence_window):
            stats["ala_loss_std_window"] = float(
                np.std(
                    np.asarray(losses[-int(ala_convergence_window) :], dtype=np.float64)
                )
            )

    stats["ala_mean"] = float(final_alpha.mean().item())
    stats["ala_std"] = (
        float(final_alpha.std(unbiased=False).item())
        if final_alpha.numel() > 1
        else 0.0
    )
    stats["ala_min"] = float(final_alpha.min().item())
    stats["ala_max"] = float(final_alpha.max().item())

    # Per-task diagnostics and visible support counts.
    y = runtime.client.train_h["n"].y.float()
    if "label_mask" in runtime.client.train_h["n"]:
        mask = runtime.client.train_h["n"].label_mask.float()
    else:
        mask = torch.ones_like(y)
    visible_pos = (y * mask).sum(dim=0)[: int(num_tasks)].detach().cpu().float()
    for task_idx in range(int(num_tasks)):
        stats[f"ala_task{task_idx}_mean"] = float(final_alpha[int(task_idx)].item())
        stats[f"ala_task{task_idx}_std"] = 0.0
        stats[f"ala_task{task_idx}_visible_pos"] = float(
            visible_pos[int(task_idx)].item()
        )
        stats[f"ala_task{task_idx}_init_alpha"] = float(
            init_alpha.detach().cpu()[int(task_idx)].item()
        )

    if debug:
        print(
            f"[TaskLevel-APPLE-ALA DEBUG] round={round_idx} "
            f"client={runtime.client.graph_id} alpha={final_alpha.tolist()} "
            f"steps={step_count} init={init_source}"
        )

    expanded_weights = _task_level_weights_from_alpha(template_model, final_alpha)
    return expanded_weights, stats
...
def create_apple_run_paths(
    ...
    algo_raw = _exp_algorithm()
    algo_short = {
        "apple_ala_guarded_multi_select": "apple_ala_g",
        "taskexpert_apple_expertprox_multi_select": "te_apple_xprox",
        "taskexpert_apple_ala_guarded_multi_select": "te_ala_g",
    }.get(algo_raw, algo_raw)

    subset_short = compact_subset_id_for_filename(subset_id)
    model_short = compact_model_tag_for_filename(model_tag)
    ...
    if len(stem) > 180:
        ...
        stem = (
            f"{algo_short}_{subset_short}_r{rounds}_e{local_epochs}_seed{seed}_{digest}"
        )
...
def initialize_dr_vectors(
    ...
    *,
    ...
    if init in {"sample_size", "p0"}:
        ...
        "taskexpert",
        "task_expert",
        "visible_task_support",
    }:
        if device is None:
            device = p0.device
        backbone_base = p0.detach().clone()
        task_base = (
            make_visible_support_task_matrix(
                runtime_clients,
                int(num_tasks),
                p0,
                device=device,
...
def mixed_parameter_dict(
    ...
    *,
    ...
    ala_alpha_min: float = 0.0,
    ala_alpha_max: float = 1.0,
) -> Dict[str, torch.Tensor]:
    """
    ...
    for name in names:
        ...
        if ala_weights is not None and name in ala_weights:
            ...
            alpha = (
                ...
                .clamp(float(ala_alpha_min), float(ala_alpha_max))
            )
...
def apple_forward(
    ...
    *,
    ...
    params = mixed_parameter_dict(
        ...
        ala_alpha_min=ala_alpha_min,
        ala_alpha_max=ala_alpha_max,
    )
...
def train_epoch_neighbor_apple(
    ...
    *,
    ...
    lambda_r: float,
    ala_weights: Optional[Dict[str, torch.Tensor]] = None,
    task_prox_target: Optional[torch.Tensor] = None,
    ala_alpha_min: float = 0.0,
    ala_alpha_max: float = 1.0,
) -> Dict[str, float]:
    receiver_model = models_for_mix[client_idx]
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        # Regularize the learned task-wise DR rows toward a configurable target.
        # For TaskExpert runs this target is the initial visible-support task
        # prior; for non-TaskExpert APPLE-ALA it resolves to the usual p0 prior.
        task = dr_vector.task
        if task_prox_target is None:
            prox_target = p0.unsqueeze(0).to(device=task.device, dtype=task.dtype)
        else:
            prox_target = task_prox_target.to(device=task.device, dtype=task.dtype)

        task_row_l2 = torch.sum((task - prox_target) ** 2, dim=1)
...
def append_apple_train_diag_row(
    ...
    *,
    ...
    if task_prox_target is None:
        task_target_cpu = p0_cpu.unsqueeze(0)
    else:
        task_target_cpu = task_prox_target.detach().cpu()
    task_l2_to_prox_target = torch.norm(task_cpu - task_target_cpu, p=2, dim=1)
    ...
    rows.append(
        {
            ...
            "dr_l2_to_prox_target": float(task_l2_to_prox_target.mean().item()),
            ...
            "dr_l2_to_prox_target_by_task_json": json.dumps(
                [float(x) for x in task_l2_to_prox_target.tolist()]
            ),
...
def run_apple_experiment(
    ...
    *,
    ...
    ala_start_round = int(cfg.get("apple_ala_start_round", 1))
    ala_alpha_min = float(cfg.get("apple_ala_alpha_min", 0.0))
    ala_alpha_max = float(cfg.get("apple_ala_alpha_max", 1.0))
    ala_task_level = bool(cfg.get("apple_ala_task_level", False))
    ala_support_aware_init = bool(cfg.get("apple_ala_support_aware_init", False))
    expert_prox_target = (
        str(cfg.get("apple_expert_prox_target", "auto")).lower().strip()
    )
    ...
    initial_task_dr_matrix = dr_matrix_cpu(dr_vectors).to(
        device=device, dtype=torch.float32
    )
    if expert_prox_target in {"auto", "initial", "expert", "expert_prior", "init"}:
        use_initial_task_prox = expert_prox_target != "auto" or str(
            dr_init
        ).lower().strip() in {
            "visible_support",
            "taskexpert",
    ...
    elif expert_prox_target in {"p0", "sample_size", "none"}:
        use_initial_task_prox = False
    else:
        raise ValueError(
            "apple_expert_prox_target must be one of auto, initial/expert_prior, p0/sample_size, none; "
            f"got {expert_prox_target!r}"
        )
    if use_initial_task_prox:
        task_prox_targets = [
            initial_task_dr_matrix[i].detach().clone() for i in range(num_clients)
        ]
        prox_target_name = "initial_task_prior"
    else:
        p0_task_target = p0.detach().clone().unsqueeze(0).repeat(int(num_tasks), 1)
        task_prox_targets = [
            p0_task_target.detach().clone() for _ in range(num_clients)
        ]
        prox_target_name = "p0_sample_size"
    ...
    print(
        ...
        ala_start_round,
        "ala_alpha_bounds=",
        (ala_alpha_min, ala_alpha_max),
        "ala_task_level=",
        ala_task_level,
        "ala_support_aware_init=",
        ala_support_aware_init,
    )
    ...
    best_by_eval_mode = {
        eval_mode: {metric: _initial_best_record() for metric in selection_metrics}
    ...
    for round_idx in range(1, rounds + 1):
        ...
        ala_enabled_this_round = bool(
            use_head_ala and round_idx >= int(ala_start_round)
        )
        ...
        for idx in selected_indices:
            ...
            if ala_enabled_this_round:
                ...
                if ala_task_level:
                    expanded_ala_weights, ala_stats = (
                        learn_task_level_ala_weights_for_client(
                            client_idx=idx,
                            runtime=runtime,
                            cfg=cfg,
                            ctx=ctx,
                            ...
                            alpha_min=ala_alpha_min,
                            alpha_max=ala_alpha_max,
                            support_aware_init=ala_support_aware_init,
                            device=device,
                            round_idx=round_idx,
                            debug=ala_debug,
                        )
                    )
                    ala_weights_for_idx = _copy_ala_weights(expanded_ala_weights)
                else:
            ...
            for local_epoch_idx in range(1, local_epochs + 1):
                train_stats = train_epoch_neighbor_apple(
                    ...
                    task_prox_target=task_prox_targets[idx],
                    ala_alpha_min=ala_alpha_min,
                    ala_alpha_max=ala_alpha_max,
                )
                append_train_row(
        ...
        for eval_mask_mode in ("full", "visible"):
            for idx, runtime in enumerate(runtime_clients):
                ala_weights_for_idx = (
                    current_ala_weights_by_client.get(idx)
                    if ala_enabled_this_round
                    else None
                )
        ...
        for eval_mask_mode in ("full", "visible"):
            ...
            for selector_metric in selection_metrics:
                ...
                if is_better_selection_value(
                    current_value,
                    (
                        initial_selection_value(selector_metric)
                        if best_record["value"] is None
                        else best_record["value"]
                    ),
                    selector_metric,
                ):
    ...
    checkpoint = {
        ...
        "best_round_by_eval_mode_and_metric": {
            mode: {
                metric: best_by_eval_mode[mode][metric]["round"]
        ...
        "best_value_by_eval_mode_and_metric": {
            mode: {
                metric: best_by_eval_mode[mode][metric]["value"]
                for metric in selection_metrics
            }
            for mode in ("full", "visible")
        },
        ...
        "apple_ala_start_round": ala_start_round,
        "apple_ala_alpha_min": ala_alpha_min,
        "apple_ala_alpha_max": ala_alpha_max,
        "apple_ala_task_level": ala_task_level,
        "apple_ala_support_aware_init": ala_support_aware_init,
        "apple_expert_rho": expert_rho,
        "apple_expert_pseudo_count": expert_pseudo_count,
        "apple_expert_prox_target": expert_prox_target,
        "apple_expert_prox_target_resolved": prox_target_name,
        "eval_protocols": [
            "oracle_full",
            "realistic_visible",
    ...
    print(
        "best rounds by eval mode and selector:",
        {
    ...
    print(
        "best values by eval mode and selector:",
        {
