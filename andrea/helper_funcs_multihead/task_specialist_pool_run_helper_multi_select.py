...
class TaskSpecialistPoolRunPaths:
    csv_path: Path
    ckpt_path: Path
    dr_csv_path: Path
...
def _algorithm() -> str:
    return os.environ.get(
        ...
        "task_specialist_pool_apple_post_ala_support_simplex_multi_select",
    )
...
def create_specialist_run_paths(
    ...
    return TaskSpecialistPoolRunPaths(
...
def compute_specialist_support_routing(
    ...
    *,
    ...
    """Return masked support prior S [task, donor] and rho [receiver, task].

    Only donors allowed by ``allowed_mask`` can receive routing mass.
    For TaskSpecialistPool, every task has exactly four allowed donors:
    the four clients assigned to that task across the physical communities.
    """
    ...
    num_tasks = receiver_visible_pos.size(1)
    num_donors = donor_visible_pos.size(0)

    if tuple(allowed_mask.shape) != (num_tasks, num_donors):
        raise ValueError(
            f"allowed_mask shape={tuple(allowed_mask.shape)}, "
            f"expected {(num_tasks, num_donors)}"
        )

    counts = allowed_mask.sum(dim=1)
    if not torch.all(counts == 4):
        raise ValueError(
            "TaskSpecialistPool expects exactly four donors per task; "
    ...
    donor_by_task = donor_visible_pos.T
    mask_f = allowed_mask.to(
        device=donor_by_task.device,
        dtype=donor_by_task.dtype,
    )

    # Support prior is restricted exactly to same-task specialists.
    score = (donor_by_task + c) * mask_f
    support = score / score.sum(dim=1, keepdim=True).clamp_min(1e-12)

    # rho compares receiver support with the strongest eligible specialist
    # for the corresponding task.
    masked_support = donor_by_task.masked_fill(~allowed_mask, float("-inf"))
    donor_task_max = masked_support.max(dim=1).values
    if not torch.isfinite(donor_task_max).all():
        raise RuntimeError("A task has no finite eligible specialist support")
...
def initialize_specialist_dr_vectors(
    ...
    *,
    ...
    """Initialize TaskSpecialistPool routing.

    All 20 clients remain available as unique donor identities, but for task t
    only the four clients whose assigned_task is t are eligible.
    """
    donor_runtimes = [
        runtime_clients[int(i)] for i in representative_indices
    ]
    ...
    allowed_mask = torch.zeros(
        (int(num_tasks), len(representative_indices)),
        dtype=torch.bool,
        device=device,
    )

    for donor_slot, donor_client_idx in enumerate(representative_indices):
        runtime = runtime_clients[int(donor_client_idx)]
        gid = str(runtime.client.graph_id)
        assigned = str(
            client_metadata.get(gid, {}).get("assigned_task", "")
        )
        if assigned not in TASKS:
            raise ValueError(
                f"Client {gid} has invalid assigned_task={assigned!r}"
            )
        task_idx = TASKS.index(assigned)
        allowed_mask[task_idx, donor_slot] = True

    per_task_counts = allowed_mask.sum(dim=1)
    if not torch.all(per_task_counts == 4):
        raise ValueError(
            "Expected four same-task specialists per task, got "
            f"{per_task_counts.detach().cpu().tolist()}"
        )

    receiver_support = _visible_positive_support_matrix(
        runtime_clients, num_tasks, device
    )
    donor_support = receiver_support[
        torch.tensor(
            list(representative_indices),
            dtype=torch.long,
            device=device,
        )
    ]
    ...
    # p0 remains the APPLE initialization. The masked softmax below
    # renormalizes it only over the four eligible donors for each task.
    base_logits = torch.log(p0.clamp_min(1e-12))
    task_logits_base = base_logits.unsqueeze(0).repeat(int(num_tasks), 1)

    print(
        "[TaskSpecialistPool] donor indices:",
        list(representative_indices),
    )
    print(
        "[TaskSpecialistPool] eligible donors per task:",
        per_task_counts.detach().cpu().tolist(),
    )
    print(
        "[TaskSpecialistPool] DR storage shape:",
        (len(runtime_clients), num_tasks, len(representative_indices)),
    )
    print("[TaskSpecialistPool] support prior S [task,20]:")
    print(support.detach().cpu())
    print("[TaskSpecialistPool] rho [20,task]:")
...
# These three functions intentionally shadow the generic PostALA versions
# imported above. The support mask determines which four donors are legal
# for each task.
def learned_task_weights(dr_vector: AppleDRState) -> torch.Tensor:
    mask = dr_vector.support > 0
    logits = dr_vector.task_logits.masked_fill(
        ~mask,
        torch.finfo(dr_vector.task_logits.dtype).min,
    )
...
def effective_task_weights(dr_vector: AppleDRState) -> torch.Tensor:
    learned = learned_task_weights(dr_vector)
    support = dr_vector.support.to(
        device=learned.device,
        dtype=learned.dtype,
    )
    rho = dr_vector.rho.to(
        device=learned.device,
        dtype=learned.dtype,
    ).unsqueeze(1)
...
def dr_matrix_cpu(
    ...
    return torch.stack(
        [
            effective_task_weights(state).detach().cpu().clone()
            for state in dr_vectors
        ],
        dim=0,
    )
...
def train_dr_epoch(
    ...
    *,
    ...
    """Fit routing logits with exactly four eligible same-task donors per task."""
                ...
                rows.append(
                    {
                        ...
                        "is_fixed_representative": 0,
                        "is_eligible_same_task_specialist": int(
                            str(donor_meta.get("assigned_task", "")) == str(task_name)
                        ),
                        "p_task_itj": value,
                        ...
                        "routing_mode": "task_specialist_pool_same_task_masked_support_simplex",
                    }
                )
...
def run_task_specialist_pool_experiment(
    subset_clients: List[ClientData],
    cfg: Dict,
    seed: int,
    run_paths: TaskSpecialistPoolRunPaths,
    *,
    rounds: int,
    warmup_rounds: int,
    local_epochs: int,
    ...
    if len(subset_clients) != 20:
        raise ValueError(
            "TaskSpecialistPool is defined for exactly 20 clients; "
    ...
    if abs(float(client_fraction) - 1.0) > 1e-12:
        raise ValueError(
            "TaskSpecialistPool currently requires CLIENT_FRACTION=1.0 so all "
    ...
    for round_idx in range(1, warmup_rounds + 1):
        ...
        core_models = updated
        print(
            f"[TaskSpecialistPool] warmup round {round_idx}/{warmup_rounds} "
    ...
    # TaskSpecialistPool does not select one representative per task.
    # Keep all 20 clients as donor identities. The Support-Simplex mask
    # later restricts each task to exactly its four same-task specialists.
    representative_indices = list(range(len(runtime_clients)))

    pool_rows = []
    for donor_idx, runtime in enumerate(runtime_clients):
        gid = str(runtime.client.graph_id)
        meta = client_metadata.get(gid, {})
        assigned = str(meta.get("assigned_task", ""))
        if assigned not in TASKS:
            raise ValueError(
                f"Client {gid} has invalid assigned_task={assigned!r}"
            )
        pool_rows.append(
            {
                "donor_client_idx": donor_idx,
                "donor_graph_id": gid,
                "donor_dataset_id": str(runtime.client.dataset_id),
                "donor_assigned_task": assigned,
                "donor_task_idx": TASKS.index(assigned),
                "donor_physical_community": meta.get("physical_community"),
                "pool_mode": "all_four_same_task_specialists",
            }
        )
    ...
    print(
        "[TaskSpecialistPool] fixed donor pool contains all 20 clients; "
        "exactly four are eligible for each task."
    )
    ...
    print("[TaskSpecialistPool] DR tensor shape:", actual_shape)
    ...
    # ------------------------------------------------------------------
    # Phase 2: all clients train locally; 20 backbones FedAvg; four eligible heads per task.
    # ------------------------------------------------------------------
    for round_idx in range(warmup_rounds + 1, rounds + 1):
        ...
        for idx, runtime in enumerate(runtime_clients):
            ...
            # B. Fit task routing; only four same-task specialists are eligible.
            ...
            # C. Mix the four eligible same-task specialist heads, then head-only PostALA
        ...
        print(
            f"[TaskSpecialistPool] collaboration round {round_idx}/{rounds} "
            f"time={format_seconds(time.perf_counter() - start)}"
        )

    # The task-specialist pool itself is fixed from known task roles,
    # so one training run can report all three evaluation protocols.
    final_jobs = [
        ("realistic_visible", "visible", "visible"),
        ("realistic_selection_oracle", "visible", "full"),
        ("oracle_full", "full", "full"),
    ]
    ...
    checkpoint = {
        ...
        "donor_client_indices": list(representative_indices),
        "donor_graph_ids": [
            str(runtime_clients[int(i)].client.graph_id)
            for i in representative_indices
        ],
        "specialist_pool_mode": "all_four_same_task_specialists",
        "eligible_donors_per_task": 4,
    ...
    print("saved task-specialist pool map ->", run_paths.representatives_csv_path)


def run_task_specialist_pool(
    subset_clients: List[ClientData],
    cfg: Dict,
    seed: int,
    root: str,
    *,
    ...
    run_task_specialist_pool_experiment(
        subset_clients,
        cfg,
        seed,
        paths,
