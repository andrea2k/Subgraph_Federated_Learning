...
def create_apple_run_paths(
    ...
    stem = (
        f"apple_fedavg_backbone_qinit_taskhead"
        f"{subset_id}"
        f"_rounds{rounds}"
...
def build_apple_log_row(
    *,
    ...
    return {
        "run_type": "apple_fedavg_backbone_qinit_taskhead",
        "algorithm": "apple_fedavg_backbone_qinit_taskhead",
        "subset_id": subset_id,
        ...
        "apple_mixing_mode": cfg.get(
            "apple_mixing_mode", "fedavg_backbone_qinit_task_head"
        ),
...
def initialize_dr_vectors(
    ...
    *,
    ...
    """
    Q-initialized learned task-head DR initialization.

    For every receiver client i we initialize:

        task_i[t, j] = oracle-q share of donor j for label task t

    But unlike fixed QOracle, task_i is learnable:
        requires_grad=True

    Backbone DR is still only a p0/debug vector. The real backbone sharing is
    done by FedAvg, not by APPLE DR mixing.
    """
    ...
    for _ in range(len(runtime_clients)):
        out.append(
            AppleDRState(
                # Backbone DR is not used for optimization/mixing in this method.
                backbone=torch.nn.Parameter(
                    backbone_base.clone(),
                    requires_grad=False,
                ),
                # This is the important change:
                # start from oracle-q, but allow APPLE to learn/update it.
                task=torch.nn.Parameter(
                    oracle_task.clone(),
                    requires_grad=True,
                ),
...
def make_apple_optimizer(
    ...
    """
    Optimize:
      1. local model parameters with normal model lr
      2. task-head DR matrix with dr_lr

    Backbone DR is intentionally not optimized because the backbone is handled
    by FedAvg aggregation.
    """
...
def train_epoch_neighbor_apple(
    ...
    *,
    ...
    for batch_idx, batch in enumerate(loader, start=1):
        ...
        # Learn task-wise DR starting from the oracle-q initialization.
        # This is the same APPLE-style proximal term as the learned task-head
        # version: it prevents the learned DR matrix from drifting too wildly.
        task = dr_vector.task
...
def run_apple_experiment(
    ...
    *,
    ...
    if str(dr_constraint).lower().strip() not in {
        ...
        raise ValueError(
            "This q-init implementation expects a learnable DR constraint, "
            "for example dr_constraint='unconstrained'."
        )
    ...
    for round_idx in range(1, rounds + 1):
        ...
        for eval_mask_mode in ("full", "visible"):
            ...
            if current_value < best_by_eval_mode[eval_mask_mode]["value"]:
                ...
                best_by_eval_mode[eval_mask_mode][
                ...
                best_by_eval_mode[eval_mask_mode][
                    "backbone_dr_matrix"
                ] = dr_backbone_matrix_cpu(dr_vectors)
    ...
    default_backbone_dr_matrix = default_state["backbone_dr_matrix"]

    def make_eval_dr_states(task_matrix: torch.Tensor, backbone_matrix: torch.Tensor):
        return [
            AppleDRState(
                backbone=torch.nn.Parameter(backbone_matrix[i].to(device).clone()),
                task=torch.nn.Parameter(task_matrix[i].to(device).clone()),
            )
            for i in range(task_matrix.size(0))
        ]
    ...
    state_by_eval_mode = {
        mode: {
            ...
            "backbone_dr_matrix": best_by_eval_mode[mode][
                "backbone_dr_matrix"
            ].clone(),
        }
    ...
    checkpoint = {
        ...
        "apple_mixing_mode": cfg.get("apple_mixing_mode", "fedavg_backbone_qinit_task_head"),
        "output_head": cfg.get("output_head", "multi"),
        "num_tasks": num_tasks,
        "backbone_dr_vectors": [
            default_backbone_dr_matrix[i].clone()
            for i in range(default_backbone_dr_matrix.size(0))
        ],
        "task_dr_vectors": [
        ...
        "backbone_dr_matrix": default_backbone_dr_matrix.clone(),
        "p0": p0.detach().cpu().clone(),
        "best_task_dr_matrix": default_dr_matrix.clone(),
        "task_dr_initialization": "oracle_q",
        "task_dr_learned": True,
        "oracle_task_weight_source": "q_assigned_share/q_other_share from ClientData.mask_meta",
        "cfg": cfg,
        "seed": seed,
    ...
    for job in final_eval_jobs:
        ...
        selected_dr_vectors = make_eval_dr_states(
            selected_state["dr_matrix"],
            selected_state["backbone_dr_matrix"],
        )
    ...
    print("\n" + "=" * 100)
    print("APPLE FEDAVG-BACKBONE QINIT TASK-HEAD DUAL-EVALUATION SMOKE CHECK")
    print("=" * 100)
