...
def run_apple_experiment(
    ...
    *,
    ...
    best_backbone_dr_matrix = dr_backbone_matrix_cpu(dr_vectors)
    ...
    for round_idx in range(1, rounds + 1):
        ...
        # Upload locally updated models.
        # Heads remain client-specific; backbone will be averaged below.
    ...
    best_dr_vectors = [
        AppleDRState(
            backbone=torch.nn.Parameter(
                best_backbone_dr_matrix[i].to(device).clone(), requires_grad=False
            ),
            task=torch.nn.Parameter(
                best_dr_matrix[i].to(device).clone(), requires_grad=False
            ),
        )
    ...
    checkpoint = {
        ...
        "num_tasks": num_tasks,
        "backbone_dr_vectors": [
            best_backbone_dr_matrix[i].clone()
            for i in range(best_backbone_dr_matrix.size(0))
        ],
        "task_dr_vectors": [
            best_dr_matrix[i].clone() for i in range(best_dr_matrix.size(0))
        ],
        # Task shape [num_clients, num_tasks, num_clients].
        "dr_vectors": [
            best_dr_matrix[i].clone() for i in range(best_dr_matrix.size(0))
        ],
        "dr_matrix": best_dr_matrix.clone(),
        "dr_tensor": best_dr_matrix.clone(),
        # Backbone shape [num_clients, num_clients].
        "backbone_dr_matrix": best_backbone_dr_matrix.clone(),
        "p0": p0.detach().cpu().clone(),
        "oracle_task_matrix": best_dr_matrix[0].clone() if best_dr_matrix.ndim == 3 else best_dr_matrix.clone(),
