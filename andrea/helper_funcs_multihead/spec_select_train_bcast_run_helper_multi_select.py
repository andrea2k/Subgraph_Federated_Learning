...
def _algorithm() -> str:
    return os.environ.get(
        ...
        "spec_select_train_bcast_apple_post_ala_support_simplex_multi_select",
    )
...
def select_support_representatives(
    runtime_clients,
    ...
    *,
    ...
    """Select one representative per known task-specialist group at round 0.

    For each task t:
      1. use assigned_task metadata to identify the four known specialists;
      2. compute each candidate's visible positive TRAIN support s_{j,t};
      3. select argmax_j s_{j,t};
      4. break exact ties deterministically by smaller graph_id.

    No validation set, oracle labels, or trained model performance is used.
    """
    support = _visible_positive_support_matrix(
        runtime_clients,
        num_tasks,
        device,
    )
    ...
    for task_idx, task_name in enumerate(TASKS):
        ...
        for idx, runtime in enumerate(runtime_clients):
            ...
            assigned = str(
                client_metadata.get(gid, {}).get("assigned_task", "")
            )
        ...
        if len(candidates) != 4:
            raise ValueError(
                "Expected exactly four known specialists for "
        ...
        for idx in candidates:
            gid = str(runtime_clients[idx].client.graph_id)
            visible_pos = float(
                support[idx, task_idx].detach().cpu().item()
            )

            # Larger support wins. On an exact tie, smaller graph_id wins.
            scored.append(
                (
                    visible_pos,
                    -int(gid),
        ...
        rep_idx = int(scored[0][2])
        selected.append(rep_idx)

        for visible_pos, neg_gid, idx in scored:
            ...
            rows.append(
                {
                    "selection_source":
                        "visible_train_positive_support",
                    "selection_scope":
                        "argmax_within_known_specialist_group",
                    "selection_round": 0,
                    "task_idx": int(task_idx),
                    "task": str(task_name),
                    "candidate_client_idx": int(idx),
                    "candidate_graph_id": gid,
                    "candidate_dataset_id":
                        str(runtime.client.dataset_id),
                    "candidate_physical_community":
                        meta.get("physical_community"),
                    "candidate_assigned_task":
        ...
        print(
            ...
            "visible_train_positive_support",
            float(support[rep_idx, task_idx].detach().cpu().item()),
        )

    if len(selected) != len(TASKS):
        raise RuntimeError(
            f"Expected {len(TASKS)} representatives, got {selected}"
        )

    if len(set(selected)) != len(TASKS):
        raise RuntimeError(
            f"Representative indices are not unique: {selected}"
        )
...
def run_spec_select_train_bcast_experiment(
    ...
    *,
    ...
    if len(subset_clients) != 20:
        raise ValueError(
            "SpecSelectTrainBcast is defined for exactly 20 clients; "
            f"got {len(subset_clients)}"
        )
    if warmup_rounds != 0:
        raise ValueError(
            f"SpecSelectTrainBcast requires warmup_rounds=0, got {warmup_rounds}"
        )
    if abs(float(client_fraction) - 1.0) > 1e-12:
        raise ValueError(
            "SpecSelectTrainBcast requires CLIENT_FRACTION=1.0 so all five "
            "selected representatives participate each training round."
        )
    ...
    # ------------------------------------------------------------------
    # Round-0 specialist selection.
    #
    # The 20 clients are used only to identify one representative inside
    # each known four-client specialist group. Selection uses visible
    # positive TRAIN support only. No model is trained before selection.
    # ------------------------------------------------------------------
    representative_indices_20, representative_rows = select_support_representatives(
        runtime_clients,
        client_metadata,
        num_tasks,
        device=device,
    ...
    pd.DataFrame(representative_rows).to_csv(
        run_paths.representatives_csv_path,
        index=False,
    )

    # Only the five selected representatives participate in FL.
    # Their order follows TASKS, so local slots 0..4 correspond to
    # cycle2..cycle6 representatives respectively.
    training_runtimes = [
        runtime_clients[int(i)]
        for i in representative_indices_20
    ]

    if len(training_runtimes) != len(TASKS):
        raise RuntimeError(
            f"Expected {len(TASKS)} training representatives, "
            f"got {len(training_runtimes)}"
        )

    representative_indices = list(range(len(training_runtimes)))

    # Start all five representatives from the same initialization.
    # No client has performed any local warm-up.
    core_models = [
        clone_model_from_state(initial_state, cfg, ctx, device)
        for _ in training_runtimes
    ]

    print(
        "[SpecSelectTrainBcast] original 20-client representative indices:",
        representative_indices_20,
    )
    print(
        "[SpecSelectTrainBcast] representative graph IDs:",
        [
            str(runtime.client.graph_id)
            for runtime in training_runtimes
        ],
    )
    print(
        "[SpecSelectTrainBcast] active training clients:",
        len(training_runtimes),
        "/",
        len(runtime_clients),
    )

    # Fixed deployment mapping:
    # every one of the 20 clients receives the full model of the
    # selected representative belonging to its assigned specialist group.
    task_to_rep_slot = {
        str(task_name): int(slot)
        for slot, task_name in enumerate(TASKS)
    }

    broadcast_rep_slot_by_client = []
    ...
    for runtime in runtime_clients:
        ...
        if assigned_task not in task_to_rep_slot:
            raise ValueError(
                f"Client {gid} has unknown assigned_task={assigned_task}"
            )

        broadcast_rep_slot_by_client.append(
            task_to_rep_slot[assigned_task]
        )

    if len(broadcast_rep_slot_by_client) != len(runtime_clients):
        raise RuntimeError(
            "Broadcast mapping does not cover all runtime clients"
        )

    print(
        "[SpecSelectTrainBcast] broadcast recipient -> rep slot:",
        broadcast_rep_slot_by_client,
    )

    # APPLE-PostALA now operates only inside the five-client
    ...
    print("[SpecSelectTrainBcast] DR tensor shape:", actual_shape)

    if actual_shape != expected_shape:
        raise RuntimeError(
            f"Expected DR shape {expected_shape}, got {actual_shape}"
        )
    ...
    current_ala_weights_by_client = {
        i: {}
        for i in range(len(training_runtimes))
    }
    ...
    best_by_eval_mode = {
        mode: {
            metric: _initial_best_record(initial_states)
    ...
    # ------------------------------------------------------------------
    # Phase 1: ONLY the five representatives train.
    # Backbone FedAvg and APPLE-PostALA are both restricted to these five.
    # ------------------------------------------------------------------
    for round_idx in range(1, rounds + 1):
        ...
        for idx, runtime in enumerate(training_runtimes):
        ...
        # E. Broadcast-style validation.
        #
        # Only five models have been trained. Each of the 20 clients is
        # evaluated using the full personalized model of the selected
        # representative from its specialist group. Recipient clients
        # perform NO fine-tuning.
        for recipient_idx, runtime in enumerate(runtime_clients):
            rep_slot = int(
                broadcast_rep_slot_by_client[recipient_idx]
            )

            broadcast_model = clone_model_from_state(
                round_local_states[rep_slot],
                cfg,
                ctx,
                device,
            )
            ...
            for eval_mode in ("full", "visible"):
                ...
                append_eval_rows(
                    ...
                    subset_clients=subset_id,
                    seed=seed,
                    phase=f"spec_select_train_bcast_val_{eval_mode}",
                    split="val",
                    graph_id=str(runtime.client.graph_id),
                    dataset_id=str(runtime.client.dataset_id),
                    metrics=metrics,
        ...
        # F. Backbone FedAvg uses ONLY the five selected representatives.
        global_backbone = fedavg_backbone_state_dict(core_models, training_runtimes)
        load_backbone_state_into_models(core_models, global_backbone)

        for eval_mode in ("full", "visible"):
            _append_mean_row(
                rows,
                phase=f"spec_select_train_bcast_val_mean_{eval_mode}",
                split="val",
                subset_id=subset_id,
                seed=seed,
                round_idx=round_idx,
        ...
        _append_bank_dr_rows(
            dr_rows,
            runtime_clients=training_runtimes,
        ...
        print(
            f"[SpecSelectTrainBcast] collaboration round {round_idx}/{rounds} "
            f"time={format_seconds(time.perf_counter() - start)}"
        )

    # Representative selection is fixed at round 0 from visible
    # TRAIN support, so the same run supports all three evaluation
    # protocols.
    final_jobs = [
        ("realistic_visible", "visible", "visible"),          # VV
        ("realistic_selection_oracle", "visible", "full"),   # VO
        ("oracle_full", "full", "full"),                     # OO
    ]
    ...
    checkpoint = {
        ...
        "representative_indices_20": list(representative_indices_20),
        "training_rep_slots": list(representative_indices),
        "representative_graph_ids": [
            str(runtime.client.graph_id)
            for runtime in training_runtimes
        ],
        "representative_selection":
            "argmax_visible_train_support_within_known_specialist_group",
        "broadcast_rep_slot_by_client":
            list(broadcast_rep_slot_by_client),
        "broadcast_source_graph_id_by_client": [
            str(training_runtimes[int(slot)].client.graph_id)
            for slot in broadcast_rep_slot_by_client
        ],
    ...
    for selector_metric in selection_metrics:
        for protocol, selected_by, eval_mode in final_jobs:
            ...
            for idx, runtime in enumerate(runtime_clients):
                rep_slot = int(
                    broadcast_rep_slot_by_client[idx]
                )

                model = clone_model_from_state(
                    selected_state["local_states"][rep_slot],
                    cfg,
                    ctx,
                    device,
                )
...
def run_spec_select_train_bcast(
    subset_clients: List[ClientData],
    cfg: Dict,
    seed: int,
    root: str,
    *,
    ...
    run_spec_select_train_bcast_experiment(
        subset_clients,
        cfg,
        seed,
        paths,
