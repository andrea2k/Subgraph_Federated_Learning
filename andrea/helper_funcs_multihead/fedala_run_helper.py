...
def create_fedala_run_paths(
    ...
    ckpt_root = ensure_dir(Path(fedala_root).parent / "checkpoints")
...
def run_fedala_experiment(
    ...
    *,
    ...
    best_round = -1
    best_global_state = clone_state_dict_cpu(global_model.state_dict())
    best_local_states = [clone_state_dict_cpu(state) for state in local_states]
    ...
    for round_idx in range(1, rounds + 1):
        ...
        for idx, runtime in enumerate(runtime_clients):
            ...
            personalized_val_infos.append(metrics)
            append_eval_rows(
                rows,
                run_type=algorithm,
                algorithm=algorithm,
        ...
        personalized_mean = _append_mean_eval_row(
            rows,
            run_type=algorithm,
            algorithm=algorithm,
            subset_id=subset_id,
            subset_clients_str=subset_clients_str,
            seed=seed,
            phase="personalized_val_mean",
            split="val",
            round_idx=round_idx,
            eval_infos=personalized_val_infos,
        )

        current_value = personalized_mean[selection_metric]
        if debug:
            print("\n[FedALA ROUND SUMMARY]")
            print(f"  round={round_idx}")
            print(
                f"  personalized val loss={personalized_mean['loss']:.6f} "
                f"micro_f1={personalized_mean['micro_f1']:.4f} "
                f"macro_pos_f1={personalized_mean['macro_pos_f1']:.4f}"
            )
            print(
                f"  global val       loss={global_mean['loss']:.6f} "
                f"micro_f1={global_mean['micro_f1']:.4f} "
                f"macro_pos_f1={global_mean['macro_pos_f1']:.4f}"
            )
            print(
                f"  selection {selection_metric}: current={current_value:.6f}, best={best_value:.6f}"
            )
        ...
        if current_value < best_value:
            ...
            best_local_states = [clone_state_dict_cpu(state) for state in local_states]
            if debug:
                print(f"  NEW BEST personalized checkpoint at round {best_round}")
    ...
    checkpoint = {
        ...
        "personalized_state_dicts": best_local_states,
        "cfg": cfg,
        "seed": seed,
    ...
    # Final evaluation: both personalized models and the corresponding global model.
    global_model.load_state_dict(best_global_state, strict=True)

    for split_name in ["train", "val", "test"]:
        for idx, runtime in enumerate(runtime_clients):
            # Personalized local model.
            metrics = _evaluate_state_on_runtime(
                state=best_local_states[idx],
                runtime=runtime,
                cfg=cfg,
                ctx=ctx,
                device=device,
            ...
            append_eval_rows(
                ...
                subset_clients=subset_clients_str,
                seed=seed,
                phase=f"best_personalized_{split_name}",
                split=split_name,
                graph_id=str(runtime.client.graph_id),
                dataset_id=str(runtime.client.dataset_id),
                metrics=metrics,
            ...
            # Global server model diagnostic.
            loader = {
                "train": runtime.train_loader,
                "val": runtime.val_loader,
                "test": runtime.test_loader,
            }[split_name]
            global_metrics = evaluate_loader(
                global_model,
                loader,
                runtime.criterion,
                device,
