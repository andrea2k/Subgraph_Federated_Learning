...
def run_apple_experiment(
    ...
    *,
    ...
    best_dr_vectors = [
        AppleDRState(
            backbone=torch.nn.Parameter(best_backbone_dr_matrix[i].to(device).clone()),
            task=torch.nn.Parameter(best_dr_matrix[i].to(device).clone()),
        )
    ...
    checkpoint = {
        ...
        "apple_mixing_mode": "fedavg_backbone_task_head",
        "output_head": cfg.get("output_head", "multi"),
        "num_tasks": num_tasks,
        "backbone_dr_vectors": [
